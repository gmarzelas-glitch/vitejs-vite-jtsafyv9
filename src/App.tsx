...
import { LayoutDashboard, Camera, FileText, Trash2, X, Settings as SettingsIcon, Save, Paperclip, Loader2, User as UserIcon, FolderOpen, MapPin, Briefcase, Send } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="flex flex-col lg:flex-row h-screen bg-[#f8fafc] font-sans overflow-hidden text-[#0f172a] text-[11px]">
      <aside className="w-full lg:w-64 bg-[#0f172a] text-white p-4 lg:p-6 flex lg:flex-col border-b lg:border-r border-slate-800 justify-between lg:justify-start">
        <div className="text-2xl lg:text-4xl font-black text-[#dc2626] italic uppercase lg:mb-12">DDF</div>
        <nav className="flex lg:flex-col gap-2 text-[9px] lg:text-[10px] font-black uppercase italic">
          <div className="p-2 lg:p-4 bg-[#dc2626] rounded-xl flex gap-2 items-center"><LayoutDashboard size={18}/> Dashboard</div>
          <button onClick={() => setShowSettings(true)} className="p-2 lg:p-4 hover:bg-slate-900 rounded-xl flex gap-2 items-center"><SettingsIcon size={18}/> Bank Details</button>
        </nav>
      </aside>
      ...
      <main className="flex-1 p-4 lg:p-8 overflow-y-auto relative">
        <div className="max-w-5xl mx-auto">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-8 gap-4 font-black italic uppercase">
            <h2 className="text-4xl lg:text-6xl tracking-tighter border-l-8 lg:border-l-[15px] border-[#dc2626] pl-4 lg:pl-8 leading-none">
              Expenses <span className="text-slate-300">#{reportCounter}</span>
            </h2>
            <button onClick={() => document.getElementById('uInput')?.click()} className="w-full sm:w-auto bg-[#dc2626] text-white px-8 py-5 rounded-[2rem] shadow-xl hover:bg-slate-950 transition-all flex items-center justify-center gap-3">
              {isAnalyzing ? <Loader2 className="animate-spin" size={20}/> : <Camera size={20}/>} Scan
            </button>
          </div>

          {/* Identity Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8 bg-white p-6 rounded-[2.5rem] border-2 border-slate-100 shadow-sm font-black italic uppercase">
             {[
          ...
          <div className="bg-white border-[6px] border-[#0f172a] rounded-2xl lg:rounded-[3.5rem] overflow-hidden mb-8 shadow-2xl overflow-x-auto">
            <table className="w-full text-left font-black uppercase tracking-widest text-[10px] italic min-w-[600px]">
              ...
              <tbody className="divide-y-4 divide-slate-50">
                {expenses.length === 0 ? <tr><td colSpan={4} className="p-32 text-center text-slate-200 text-2xl opacity-40 tracking-tighter">No Data</td></tr> : 
                  expenses.map(e => (
                    <tr key={e.id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-8 text-slate-400">{e.date}</td>
                      <td className="p-8">{toGreeklish(e.merchantName)}</td>
                      <td className="p-8 text-right text-xl text-[#0f172a]">€{e.totalAmount.toFixed(2)}</td>
                      <td className="p-8 text-center"><button onClick={() => setExpenses(expenses.filter(x => x.id !== e.id))} className="text-slate-200 hover:text-[#dc2626]"><Trash2 size={24}/></button></td>
                    </tr>
                  ))}
              </tbody>
            </table>
